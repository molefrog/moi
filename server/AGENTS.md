...
- `mcp-broker.ts` lets server functions call the user's Claude Code MCP tools without a model turn: discovery reuses the `mcp.ts` status probe (resolved configs + tool lists), connections are pooled here in the parent, and `.server.ts` code reaches it via `import { mcp } from 'moi'` (a `Bun.plugin` virtual module in `functions-worker.ts`, relayed over the worker IPC channel in `functions.ts`). stdio and header-authenticated http/sse servers are callable; OAuth (`needs-auth`) servers and claude.ai connectors are rejected with explanatory errors.
