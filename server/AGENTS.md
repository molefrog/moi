...
- Agents run in the workspace directory; Claude Code and Codex use provider-native reviewed access, with moi approving escalated permission requests by default until UI approvals land. Each backend persists its own session history (see `harness/README.md`).
