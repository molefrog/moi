...
test('routes once and accepts only the selected socket response', async () => {
  ...
  const a = client(),
    b = client()
  relay.message(a, { type: 'view-tool:presence', workspaceId: 'a', viewId: 'orders', tools: [] })
  relay.message(b, { type: 'view-tool:presence', workspaceId: 'b', viewId: 'orders', tools: [] })
...
test('reads discovery metadata directly from active-view presence', () => {
  const relay = createViewToolRelay(),
    a = client()
  relay.message(a, {
    ...
    workspaceId: 'a',
    viewId: 'orders',
    tools: [{ name: 'set_filter', description: 'Set the filter', inputSchema: { type: 'object' } }]
  })
  expect(relay.list('a', 'orders')).toEqual([
    { name: 'set_filter', description: 'Set the filter', inputSchema: { type: 'object' } }
  ])
  expect(relay.hasTool('a', 'orders', 'set_filter')).toBe(true)
  expect(a.messages).toEqual([])
})

test('malformed presence clears stale discovery metadata', () => {
  const relay = createViewToolRelay(),
    a = client()
  relay.message(a, {
    type: 'view-tool:presence',
  ...
  relay.message(a, {
    ...
    workspaceId: 'a',
    viewId: 'orders',
    tools: [{ name: 'invalid name' }]
  })
  expect(relay.availability('a', 'orders')).toBe('unavailable')
  expect(relay.list('a', 'orders')).toEqual([])
})
