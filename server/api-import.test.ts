...
const originalFxListModels = fxHarness.listModels
...
afterEach(async () => {
  ...
  fxHarness.listModels = originalFxListModels
...
describe('workspace import provider', () => {
  ...
  test('imports an fx workspace and installs its bundled skills', async () => {
    const workspacePath = join(tempDir, 'fx-workspace')
    await mkdir(join(workspacePath, '.moi'), { recursive: true })
    await Bun.write(join(workspacePath, '.moi', 'package.json'), '{}\n')
    ...
    const response = await api.request('/api/workspaces', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: workspacePath, type: 'fx' })
    })

    expect(response.status).toBe(201)
    expect(((await response.json()) as WorkspaceEntry).type).toBe('fx')
    expect(
      await Bun.file(join(workspacePath, '.agents', 'skills', 'moi-workspace', 'SKILL.md')).exists()
    ).toBe(true)
  })
