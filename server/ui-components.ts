...
// moi ui-components support file — re-scopes portalled overlays.
//
// Applet styles are scoped to the applet's container ([data-applet="…"]), but
// Base UI portals render overlays into document.body, outside that container.
// This wrapper puts the scope back: a hidden marker discovers which applet
// the overlay belongs to, and the portalled children render under an element
// carrying the same data-applet attribute. Installed and rewired
// automatically by \`moi ui-components add\` — applets don't use it directly.
