...
//
// Tailwind's `group-has-*` / `peer-has-*` output arrives here already rewritten
// to the descendant form — that happens in the Tailwind plugin
// (tailwind-plugin.ts), for host and applet stylesheets alike — so scoping
// only ever prepends the container.
//
