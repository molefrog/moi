...
describe('installBundledSkills', () => {
  test('installs the applet guidance and cheat sheet for every workspace', async () => {
    await withTempDir(async dir => {
      ...
      expect(intents).toContain('function navigate(href:')
      expect(intents).toContain('function resolveHref(href:')
      expect(intents).not.toContain('focusTab')
      ...
      expect(skillMd).toContain('moi navigate')
      expect(skillMd).not.toContain('moi tabs focus')
