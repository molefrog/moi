...
describe('WidgetFrame', () => {
  test('applies the swapped widget theme without forcing dark mode', () => {
    ...
    expect(html).toContain('--primary:color-mix(in oklch, var(--background) 3%, oklch(1 0 0) 97%)')
    expect(html).toContain('--accent-foreground:var(--foreground)')
    expect(html).toContain('--border:color-mix(in oklch, var(--foreground) 15%, transparent)')
  })
})
