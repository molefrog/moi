...
function send(id: string, isNew: boolean, content: string, uploadIds?: string[]) {
  return sendAcpMessage(config, {
    ...
    isNew,
    content,
    attachments: uploadIds?.map(uploadId => ({ type: 'upload' as const, uploadId })),
    stream: true
  })
}
    ...
    replayImage.some(
      event =>
        ...
        event.turn.parts.some(
          part => part.type === 'file-attachment' && part.mediaType.startsWith('image/')
        )
    )
