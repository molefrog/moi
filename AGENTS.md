...
## Data flow
  ...
  - Prefer `.moi/workspace.json` for state that describes the workspace itself rather than the person or device using it — e.g. chat/thread settings, widget layout, connectors. Litmus test:
  anyone who clones the workspace should get the same value. ```
